...
export function homePage(c: Context, opts: HomeOptions = {}): Response {
  ...
  const tiles = Object.entries(PRESETS)
    .map(([slug, p]) => {
      ...
      return `
      <label class="tile" data-preset="${escapeHtml(slug)}" title="${escapeHtml(p.label)}">
    ...
    <form method="post" action="/make">
      ...
      <div class="card">
        ...
        <div class="custom-row">
          ...
          <input name="domain" aria-label="Custom domain" type="text" placeholder="example.com" value="${escapeHtml(customDomain)}" autocomplete="off" pattern="^[a-zA-Z0-9.-]+\\.[a-zA-Z]{2,}$" title="Enter a valid domain like example.com">
        </div>
      </div>
    </form>
...
<script>
  (function () {
    ...
    var presetTiles = document.querySelectorAll('label.tile[data-preset]');
    var DOMAIN_RE = /^[a-z0-9.-]+\\.[a-z]{2,}$/i;

    function render(isTyping) {
      var isTypingFlag = (isTyping === true);
      var rawTitle = titleInput.value;
      var tabLabel = rawTitle || 'New Tab';
      var docTitle = rawTitle || 'Tab Snitch';
      var domain = currentDomain();
      
      // Prevent network spam on invalid domain typing
      var sourceChecked = document.querySelector('input[name="source"]:checked');
      var isCustom = sourceChecked && sourceChecked.value === 'custom';
      var isValid = !isCustom || DOMAIN_RE.test(domain);
      var iconUrl = isValid ? ('/icon/' + encodeURIComponent(domain)) : '/icon/google.com';
      ...
      // Real-time dynamic URL sync
      var currentPreset = sourceChecked && sourceChecked.value.startsWith('preset:') 
        ? sourceChecked.value.slice('preset:'.length) 
        : null;
      var newPath = '/';
      if (isCustom) {
        newPath = '/d/' + encodeURIComponent(domain) + '/' + (slug || 'untitled');
      } else if (currentPreset) {
        newPath = '/' + currentPreset + '/' + (slug || 'untitled');
      }
    ...
    // Support modifier click / middle click on labels to open decoy in a new tab
    presetTiles.forEach(function (label) {
      label.addEventListener('click', function (e) {
        if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey || e.button === 1) {
          e.preventDefault();
          e.stopPropagation();
          var rawTitle = titleInput.value;
          var slug = toSlug(rawTitle) || 'untitled';
          var href = '/' + label.dataset.preset + '/' + slug;
    ...
    window.addEventListener('popstate', function () {
      ...
      if (parts.length === 2 && parts[0] !== 'd') {
        ...
        // Custom domain decoy
        var radio = document.querySelector('input[name="source"][value="custom"]');
        if (radio) {
          radio.checked = true;
          domainInput.value = decodeURIComponent(parts[1]);
          titleInput.value = parts[2] === 'untitled' ? '' : unslug(parts[2]);
        }
      } else {
        // Home/empty state
        var defaultRadio = document.querySelector('input[name="source"][value="preset:google"]');
        if (defaultRadio) {
    ...
    titleInput.addEventListener('input', function () { render(true); });
